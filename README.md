...
**Update at 2022 Feb 20 13:15 (UTC+8)**: 修改了 `captcha/online.py` 中 `TTShituRecognizer` 的 `data` 参数 (添加了 `"typeid": 1003`)，已经可以实现课程列表页面的正常跳转。

本项目 fork 自 [Somoku
/
PKUElective2022Spring
](https://github.com/Somoku/PKUElective2022Spring)，其是基于 [PKUAutoElective](https://github.com/zhongxinghong/PKUAutoElective)，对 2021 春季学期的选课网站 API 改动进行了调整。并针对验证码系统的改动，将识别系统转为在线商用平台 [TT识图](http://www.ttshitu.com)，在将识别码类型改为英数混合2 (代码`1003`)后，识别准确率与识别速度均得到了一定程度的提升。
